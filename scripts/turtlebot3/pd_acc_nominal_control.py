...
#             Ka * (
#                 Kp_v * np.linalg.norm(self.target[0:2] - x[0:2])  # v_ref
#                 - (x[3] if x.shape[0] > 3 else 0.0)                    # - v
#             ),
#             # ---- angular velocity channel ----
#             Kw * np.arctan2(
#                 np.cos(x[2]) * -(x[0] - self.target[0]) + np.sin(x[2]) * -(x[1] - self.target[1]),
#                 -np.sin(x[2]) * -(x[0] - self.target[0]) + np.cos(x[2]) * -(x[1] - self.target[1])
#             )
#         ]],
#         self.umin, self.umax
#     )
